...
def load_and_wrap_model_with_io_scaling(model):
    # Input layer with same shape as the original
    #input_layer = tf.keras.Input(shape=model.input_shape[1:], batch_size=1)
    # input_layer = tf.keras.Input(shape=(1, 512, 512, 4), batch_size=1)    

   # Input as uint8 (image format)
    input_layer = tf.keras.Input(shape=(512, 512, 4), batch_size=1, dtype=tf.uint8)

    # Keep RGB, drop alpha (assumes RGBA input)
    rgb_input = tf.keras.layers.Lambda(lambda x: x[..., :3])(input_layer)

    # Cast to float for processing
    float_input = tf.keras.layers.Lambda(lambda x: tf.cast(x, tf.float32))(rgb_input)

    # Normalize to match model training (choose your scale)
    scaled_input = tf.keras.layers.Lambda(lambda x: (x / 127.5) - 1.0)(float_input)

    # Pass through the original model
    raw_output = model(scaled_input)
 
    # Remove the last channel dimension (e.g. (None, 512, 512, 1) → (None, 512, 512))
    # squeezed_output = tf.keras.layers.Lambda(lambda x: tf.squeeze(x, axis=-1))(raw_output)

    # # Scale output to [0, 255] & convert to uint8
    # # tf.clip_by_value(x * 255.0, 0, 255
    scaled_output = tf.keras.layers.Lambda(
        lambda x: tf.cast(tf.clip_by_value(x * 255.0, 0, 255), tf.uint8)
    )(raw_output)
    # scaled_output = tf.keras.layers.Lambda(
    #     lambda x: tf.clip_by_value(x * 255.0, 0, 255)
    # )(raw_output)

    # # Final wrapped model
    wrapped_model = tf.keras.Model(inputs=input_layer, outputs=scaled_output)
...
model = tf.keras.models.load_model('/home/burk/Documents/cuda_test/newlog/512_log_quant/run_1/bs_8/dr_0.8/n_808/w_2/h_4/_model_fin.keras')
...
# Apply quantization-aware transformation
...
q_aware_model_history = q_aware_model.fit(train_dataset, epochs=ep, validation_data=val_dataset)



# builds path
path = construct_next_path()
# saves q_aware_model and matadata

plot_training_history(q_aware_model_history, path)
q_aware_model.save(path + "/" + version_name + "_q.keras")
q_aware_model.export(path + "/" + version_name + "_q_old")

# saves q_aware_model dataset
...
# creates wrapper for function and converts it to tflite
wraped_q = load_and_wrap_model_with_io_scaling(q_aware_model)

wraped_q.summary()
converter = tf.lite.TFLiteConverter.from_keras_model(wraped_q)
converter.optimizations = [tf.lite.Optimize.DEFAULT]
converter.target_spec.supported_types = [tf.uint8]
# converter.representative_dataset = representative_dataset_gen

# Input is float32, output is uint8
tflite_model = converter.convert()

tflite_path = path + "/quant_uint8.tflite"
...
wraped_model = load_and_wrap_model_with_io_scaling(model)

converter_float16 = tf.lite.TFLiteConverter.from_keras_model(wraped_model)
# converter.optimizations = [tf.lite.Optimize.DEFAULT]
# 
# converter.representative_dataset = representative_dataset_gen
...
tflite_model = converter_float16.convert()

tflite_path = path + "/quant_float16.tflite"
tf.lite.experimental.Analyzer.analyze(model_content=tflite_model, gpu_compatibility=True)
